...
#[tokio::test]
async fn query_fails_fast_when_ollama_is_disconnected() {
    let root = unique_temp_dir("query-health-guard");
    ...
    let error = execute(
        Command::Query {
            text: "find br 50".to_string(),
        },
        &context,
    )
    .await
    .expect_err("query should fail fast");
