...
#[tokio::test]
async fn scrape_fails_fast_when_ollama_is_disconnected() {
    let root = unique_temp_dir("scrape-health-guard");
    ...
    .await
    .expect_err("scrape should fail fast");

    assert!(error.to_string().contains("Ollama unavailable for /scrape"));
}
