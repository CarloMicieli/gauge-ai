...
use gauge_ai::storage::db::{initialize, list_models, persist_model};
...
#[tokio::test]
async fn export_writes_missing_image_manifest() {
    let root = unique_temp_dir("export-missing-images");
    ...
    let mut models = list_models(&pool).await.expect("list models failed");
    let model = models.first_mut().expect("seeded model missing");
    model.local_image_paths = vec!["/tmp/non-existent-image.jpg".to_string()];
    model.source_fingerprint.push_str("-missing");
    persist_model(&pool, model)
        .await
        .expect("persist model failed");
    ...
    assert_eq!(export.images, 0);
    assert_eq!(export.missing_images, 1);

    let manifest = PathBuf::from(export.output_path).join("missing_images.txt");
    assert!(manifest.exists());
    let content = std::fs::read_to_string(manifest).expect("failed to read manifest");
    assert!(content.contains("/tmp/non-existent-image.jpg"));
}
